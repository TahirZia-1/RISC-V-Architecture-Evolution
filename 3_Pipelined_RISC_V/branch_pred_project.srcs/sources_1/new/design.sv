...
module instr_mem(
    ...
    initial begin
        addr = 0;
        // I-type immediate
        load_inst(addr, 32'h00300313); addr = addr + 4; // li t1, 3 
        load_inst(addr, 32'h00400393); addr = addr + 4; // li t2, 4 
        load_inst(addr, 32'h00400e13); addr = addr + 4; // li t3, 4 
...
//        // R-type
//        load_inst(addr, 32'h007302b3); addr = addr + 4; // add t0, t1, t2 
//        load_inst(addr, 32'h407302b3); addr = addr + 4; // sub t0, t1, t2 
//        load_inst(addr, 32'h007372b3); addr = addr + 4; // and t0, t1, t2 
//        load_inst(addr, 32'h007362b3); addr = addr + 4; // or t0, t1, t2 
        

//        // S-type Store
//        load_inst(addr, 32'h00702223); addr = addr + 4; // sw t2, 4(x0) 

//        // I-type load
//        load_inst(addr, 32'h00402283); addr = addr + 4; // lw t0, 4(x0) 
        
//        // R-type: test cases for load instruction
//        load_inst(addr, 32'h006283B3); addr = addr + 4; // add t2 t0 t1 
//        load_inst(addr, 32'h406283B3); addr = addr + 4; // sub t2 t0 t1 
//        load_inst(addr, 32'h0062E3B3); addr = addr + 4; // or t2 t0 t1 




        // B-type Beq: beq t2, t3, -16    (If t2 == t3, skip next 
        // instruction. Target PC = PC - 16)
        //load_inst(addr, 32'hFFC388E3); addr = addr + 4; 
        //load_inst(addr, 32'h00730463); addr = addr + 4;


        // B-type Beq: beq t2, t3, 20    (If t2 == t3, skip next 
        // instruction. Target PC = PC + 20)
        load_inst(addr, 32'h01C38A63); addr = addr + 4; 
        //load_inst(addr, 32'h00730463); addr = addr + 4;

        // Instruction to be skipped if beq is true 
        load_inst(addr, 32'h007302b3); addr = addr + 4; // add t0, t1, t2
...
//        // B-type Beq: beq t2, t3, 8    (If t2 == t3, skip next 
//        // instruction. Target PC = PC + 8 = 28)
//        load_inst(addr, 32'h01c38463); addr = addr + 4; 
//        //load_inst(addr, 32'h00730463); addr = addr + 4;
...
//        // J-type: jal:  jal x2, 8 (jump to PC+8 (skip next instruction), 
//        // x2 = return address (PC+4))
//        load_inst(addr, 32'h0080016f); addr = addr + 4; 
//        //load_inst(addr, 32'h00730463); addr = addr + 4;
...
//        load_inst(addr, 32'h007302b3); addr = addr + 4; // add t0, t1, t2 
      	...
      	// Target destination of beq (Address 0x20)
        load_inst(addr, 32'h407302b3); addr = addr + 4; // sub t0, t1, t2 
    
        // ---- NEW TEST FOR LW STALLING ----
        
        // 1. Store t2 (which holds 4) into memory at address 0 
        // sw t2, 0(x0) 
        load_inst(addr, 32'h00702023); addr = addr + 4; 

        // 2. Load that 4 back into t0 
        // lw t0, 0(x0) 
        load_inst(addr, 32'h00002283); addr = addr + 4; 
        
        // 3. Immediately use t0! This forces a LOAD-USE STALL.
        // The processor MUST stall for 1 cycle here to wait for memory.
        // add t2, t0, t1 
        load_inst(addr, 32'h006283B3); addr = addr + 4;
...
module main_decoder(
    ...
    always_comb begin
        case(Op)
            7'b0010011: begin // I-type: immediate: ADDI, ANDI, ORI, SLTI
...
module hazard_unit(
    ...
    // data hazard : lw stalling and flushing && control hazard : beq flushing 
    always_comb begin 
        if(lwStall) begin //lw stalling 
            StallF = 1'b1;
            StallD = 1'b1; 
        end else begin
            StallF = 1'b0;
...
module control_unit(
    ...
    logic BranchE, JumpE, RegWriteE, MemWriteE, MemReadE;
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            // Register Execute to Memory
            RegWriteM   <= 1'b0;
            MemWriteM   <= 1'b0;
            MemReadM    <= 1'b0;
            ResultSrcM  <= 2'b0;
            
            // Register Memory to Writeback
            RegWriteW   <= 1'b0;
            ResultSrcW  <= 2'b0;
...
module data_path (
    ...
    mux2x1 PC_Mux (.a(PCPlus4F),.b(PCTargetE),.control(PCSrcE),.out(pc_inF));
...
//    data_path    data_path_inst    ( .* );
    ...
    control_unit control_unit_inst( .clk(clk), .rst(rst), .zeroE(zeroE), .opcodeD(opcodeD), .funct3D(funct3D),
        ...
        .funct7_5D(funct7_5D), .PCSrcE(PCSrcE), .RegWriteW(RegWriteW), .ALUSrcE(ALUSrcE), .ALUcontrolE(ALUcontrolE), 
    ...
    data_path data_path_inst ( .clk(clk), .rst(rst), .PCSrcE(PCSrcE), .RegWriteW(RegWriteW), .ALUSrcE(ALUSrcE), 
